...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x).transpose(1,2)  # BxCxHxW -> BxNxC , N=(224/4)^2=3136, C=64
...
if __name__ == '__main__':
    x=torch.randn((63,1,2560))
    model=PatchEmbed(img_size=2560,patch_size=10,in_chans=1,embed_dim=64)
    print(model(x).shape)